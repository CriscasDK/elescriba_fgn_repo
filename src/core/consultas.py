...
def ejecutar_consulta_persona(nombre_persona, limit=10, departamento=None, municipio=None, fecha_inicio=None, fecha_fin=None):
    ...
    try:
        ...
        if necesita_joins:
            ...
            where_conditions = ["p.nombre ILIKE %s", "p.tipo ILIKE %s"]
            ...
            if departamento or municipio:
                ...
                where_simple = ["p.nombre ILIKE %s", "p.tipo ILIKE %s"]
            ...
            # Sin filtros: usar lógica original con variantes (código del baseline)
            nombres_variantes = [nombre_persona]
            partes = nombre_persona.split()
            if len(partes) > 1:
                nombres_variantes.append(partes[0])

            total_menciones = 0
            documentos_relacionados = []

            for nombre_var in nombres_variantes:
                cur.execute("""
                    SELECT COUNT(*) FROM personas
                    WHERE nombre ILIKE %s AND tipo ILIKE %s
                """, (f'%{nombre_var}%', '%victim%'))
                result = cur.fetchone()
                menciones_var = result[0] if result else 0
                total_menciones += menciones_var
...
def obtener_opciones_departamento() -> List[str]:
    """Obtiene lista de departamentos disponibles con normalización"""
    try:
        conn = get_db_connection()
        cur = conn.cursor()
        cur.execute("""
            ...
            FROM (
                SELECT
                    CASE
                        ...
                        WHEN departamento ILIKE '%Bogot%' OR departamento ILIKE '%D.C%' OR departamento = 'Distrito Capital' THEN 'Bogotá D.C.'
                        WHEN departamento ILIKE '%Ces%r%' THEN 'Cesar'
                        WHEN departamento ILIKE '%Valle del Cauca%' OR departamento = 'Valle' THEN 'Valle del Cauca'
                        WHEN departamento = 'Guajira' OR departamento = 'La Guajira' THEN 'La Guajira'
                        ...
                        WHEN departamento IN ('A Coruña', 'Columbia', 'Maryland', 'Nueva York', 'Jalisco', 'La Libertad', 'Distrito de Columbia') THEN NULL
                        ELSE departamento
